...
client.once("ready", async () => {
  ...
  console.log("Bot is online. Waiting until 8:00 AM Manila time to start activity check.");

  // Runs every day at 8:00 AM Manila time
  cron.schedule(
    "0 8 * * *",
    async () => {
      console.log("Daily activity check started...");

      await finishActivityCheck();
      await startActivityCheck();

      console.log("Daily activity check posted.");
    },
    {
...
async function startActivityCheck() {
  ...
  if (currentCheckMessageId) {
    try {
      ...
      console.log("Could not delete old activity check message. It may already be deleted.");
    }
  }
...
async function finishActivityCheck() {
  ...
  guild.members.cache.forEach((member) => {
    ...
    // IMPORTANT:
    // Only check members who have the role from CHECK_ROLE_ID
